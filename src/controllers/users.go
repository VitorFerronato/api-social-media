...
func UpdatePassword(w http.ResponseWriter, r *http.Request) {
	...
	if userTokenId != userId {
		responses.Error(w, http.StatusForbidden, errors.New("cannot update other users"))
		return
	}
	...
	repository := repository.NewUserRepository(db)
	dbSavedPassword, err := repository.GetPassword(userId)
	if err != nil {
		responses.Error(w, http.StatusInternalServerError, err)
		return
	}

	if err = secutiry.VerifyPassword(dbSavedPassword, password.ActualPassword); err != nil {
		responses.Error(w, http.StatusInternalServerError, err)
		return
	}

	hashPassword, err := secutiry.Hash(password.NewPassword)
	if err != nil {
		responses.Error(w, http.StatusBadRequest, err)
		return
	}
